...
- [x] `pebl-Vs0xNh`: Investigate TestEnv dead_code allowances and test helper cleanup
...
- [x] `pebl-p3k8qhfwqu`: Refactor commands_write_tests to avoid #[path] attribute
- [x] `pebl-uj0hll5buc`: Fix stale reverse index in run_add JSON output
- [x] `pebl-czi45zargx`: Use lossy-safe path serialization in init JSON output
- [x] `pebl-6pv06nwvpl`: Align help-json guidance text with actual emitted keys
...
- [x] P3.3 `fix` command: backfill missing `created_at`, warn on unknown keys, do not remove or rewrite dependencies.
- [x] P3.3 `fix` command: backfill missing `created_at`, warn on unknown keys, do not remove or rewrite dependencies.
...
- [ ] P5.3 Graph Behavior: Terminal Dependents
  ...
  - [x] P5.3.c `show`: `blocked_by` and `blocking` exclude terminal nodes.
  - [x] P5.3.d `show`: `transitive_blocking_count` traversal stops at terminal nodes.
...
- [x] P5.5 Default list ordering and explicit `--sort` tie-breakers.
