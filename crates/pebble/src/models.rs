...
impl TaskNode {
    ...
    /// Atomically creates and writes the task content to a new file on disk.
    ///
    /// # Errors
    ///
    /// Returns an error if generating the serialized content fails, if the file
    /// already exists, or if the file creation or write operation fails.
