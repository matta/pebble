...
#[test]
fn test_batch_count_blocking_equivalence() {
    let mut nodes = HashMap::new();

    // Complex graph with:
    // - Actionable chain: A -> B -> E
    // - Terminal chain: A -> B -> C (Done) -> D
    // - Cycle: F <-> G
    // - Independent: H
    // - Non-actionable cycle: I (Done) <-> J (Done)
    ...
    let f = make_test_node("F", TaskStatus::Todo, vec!["G"]);
    let g = make_test_node("G", TaskStatus::Todo, vec!["F"]);

    let h = make_test_node("H", TaskStatus::Todo, vec![]);

    let i = make_test_node("I", TaskStatus::Done, vec!["J"]);
    let j = make_test_node("J", TaskStatus::Done, vec!["I"]);
    ...
    nodes.insert("D".to_string(), d);
    nodes.insert("E".to_string(), e);
    nodes.insert("F".to_string(), f);
    nodes.insert("G".to_string(), g);
    nodes.insert("H".to_string(), h);
    nodes.insert("I".to_string(), i);
    nodes.insert("J".to_string(), j);
    ...
    let batch_counts = graph.batch_count_blocking(&ids);

    for id in &ids {
        let node = graph.nodes.get(id).expect("node must exist");
        // batch_count_blocking only supports actionable roots, which is sufficient for get_next_tasks
        if !node.frontmatter.status.is_actionable() {
            continue;
        }

        let single_count = graph.count_blocking(id);
        let batch_count = *batch_counts.get(id).expect("batch count must exist");
        assert_eq!(
            single_count, batch_count,
            "Count mismatch for task {}: single={}, batch={}",
            id, single_count, batch_count
        );
    }
}
