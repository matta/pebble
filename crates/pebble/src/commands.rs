...
/// Emits a serialized JSON payload to standard output.
///
/// This provides a uniform way to print structured JSON objects for tools
/// that use the `--json` output flag.
...
pub fn run_next(ctx: &RunContext, limit: usize) -> Result<()> {
    ...
    if ctx.json {
        let objects: Vec<TaskObject> = tasks
            ...
            .collect();
        return emit_json(&serde_json::json!({ "tasks": objects }));
    }
