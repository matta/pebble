...
fn test_show_json_stdout_only() {
    ...
    assert!(value.get("blocked_by").is_some(), "JSON missing 'blocked_by' field");
    assert!(value.get("blocking").is_some(), "JSON missing 'blocking' field");
    assert!(value.get("deps").is_none(), "JSON still contains legacy 'deps' field");
}
