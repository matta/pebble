...
fn test_worktree_creation_from_remote_branch() {
    ...
    run_git(&["remote", "add", "origin", remote_path.to_str().unwrap()], pusher_path);
    ...
    run_git(&["remote", "add", "origin", remote_path.to_str().unwrap()], local_path);
    ...
    let output = Command::new("git")
        ...
        .output()
        .unwrap();
    assert!(output.stdout.is_empty(), "Sync branch should not exist locally yet");
    ...
    assert_eq!(branch.trim(), sync_branch, "Worktree should be on sync branch");

    // Check if content exists
    assert!(worktree_path.join("sync.txt").exists(), "Sync content should exist in worktree");
}
...
fn test_worktree_creation_orphan_fallback() {
    ...
    let worktree_path = manager.ensure_worktree().expect("Failed to create orphan worktree");
    ...
    assert!(!worktree_path.join("README.md").exists(), "Orphan branch should be empty");
}
