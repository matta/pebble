...
## 2026-03-16 - TOCTOU Race Condition in File Archival Move
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability existed in `run_archive` when checking if an archive destination path existed and then subsequently moving the task file via `fs::rename`. An attacker could create a symlink or file at the generated destination path between the check and the rename, potentially causing unintended file overwriting and data loss.
**Learning:** Checking for file existence before moving or renaming it introduces a race condition. `fs::rename` will silently overwrite the destination, compounding the vulnerability.
**Prevention:** Use atomic operations for file moves. For local file systems, try `fs::hard_link` to the destination followed by `fs::remove_file` on the source, handling `AlreadyExists` errors safely within a retry loop to find a unique name. If hard linking fails (e.g., across device boundaries), fall back to an atomic open-and-copy operation using `OpenOptions::new().write(true).create_new(true)` to guarantee the destination is not overwritten.
