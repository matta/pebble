...
## 2024-05-24 - Fix TOCTOU vulnerability in commands_archive.rs
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability existed in `commands_archive.rs` during task file archiving. The code checked `Path::exists()` to find a unique filename, then called `fs::rename()`. This left a race window where another process could create a file at the target path before the rename, causing `fs::rename()` to silently overwrite it (on POSIX systems) or fail unexpectedly.
**Learning:** Checking for file existence before performing a moving operation is an anti-pattern that creates race conditions. Standard file moving functions like `fs::rename` are not safe for generating unique, non-colliding files because they do not guarantee atomic creation.
**Prevention:** To prevent TOCTOU vulnerabilities when generating and writing to unique files, use atomic creation operations like `std::fs::OpenOptions::new().write(true).create_new(true).open(...)` in a retry loop. If `open` returns an `AlreadyExists` error, handle it by retrying with a new filename.
