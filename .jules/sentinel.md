...
## 2026-03-05 - TOCTOU Race Condition via File Rename
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability was found in the archive command. The code generated a candidate path, checked for its existence with `Path::exists()`, and then later called `fs::rename()` to move the task. If a malicious process or user created the file between the check and the rename, `fs::rename` silently overwrote it on POSIX systems.
**Learning:** The `fs::rename` operation in Rust standard library overwrites destination paths silently on POSIX systems, making it vulnerable when used in combination with pre-flight existence checks.
**Prevention:** Always implement file moves that require non-overwriting guarantees atomically. In Rust, use `std::fs::OpenOptions::new().write(true).create_new(true).open(...)` to create the destination file, securely copy the contents with `std::io::copy`, and remove the source with `std::fs::remove_file`. Avoid `fs::rename` when destination collision guarantees are required.
