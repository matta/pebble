---
id: "pebl-fFdi_z"
title: "Phase 4 Spec-Driven UX Issues"
status: "done"
created_at: "2026-02-23T01:36:06.540214+00:00"
modified_at: "2026-03-01T16:36:23.238940044+00:00"
needs: ["pebl-hRuKk1", "pebl-ez0v1cj5ai"]
tags: ["bootstrap", "self_hosted"]
---
Checklist:
- [x] P4.1 Improve `.pebble/AGENTS.md` content generated by `init`
- [x] P4.2 Fix TOCTOU slug collision with atomic create (`create_new(true)` + retry)
- [x] P4.3 Transliterate non-ASCII in `slugify`
- [x] P4.4 Add help completeness test covering every subcommand
- [x] P4.5 Forbid clippy warning suppressions in `just` checks (operator approval received)

Child Tasks:
- None currently. Promote only when Adaptive Decomposition criteria are met.
