...
4. **Tooling & Xtask**: The `check-beads` xtask must be renamed to `check-pebble`.
...
- **Preserve `.bead-whitelist`**: This file and the logic associated with it must NOT be renamed, as it represents a legacy whitelist configuration that Pebble should still respect.
